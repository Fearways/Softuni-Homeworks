...
class PerimeterAndAreaOfPolygon
{
	...
	static double PolygonPerimeter(Point[] perimeter)
	{
		...
		{
			peri += CalculateDistance(perimeter[i].X, perimeter[i].Y, perimeter[i + 1].X, perimeter[i + 1].Y);
		}
		return peri;
	}
	...
	static void Main()
	{
		...
		Console.WriteLine("perimeter = {0:F2}", PolygonPerimeter(perimeter));
		Console.WriteLine("area = {0:F2}", PolygonArea(matrix, pointsNumber));
	}
}